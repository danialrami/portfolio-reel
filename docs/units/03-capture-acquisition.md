...
`wf-recorder` on wlroots Wayland), write video media + a `capture.json`, with `--dry-run`,
`--json`, and clean signal handling. No interactive prompts. Audio capture is deferred from
phases 0/1 and must never be added implicitly.
...
- Platform research (2026): Linux X11 `ffmpeg -f x11grab`; Linux wlroots/Hyprland
  `wl-screenrec`/`wf-recorder`; macOS `ffmpeg -f avfoundation`; Windows `ffmpeg -f gdigrab`.
...
- `CaptureRequest` is a small dataclass: `region | monitor | window`, `fps`, `duration | None`,
  `codec`, `out`, `name`. Audio-shaped fields remain reserved in the document schema but are
  rejected by the phase-0/1 runner until their source semantics are verified.

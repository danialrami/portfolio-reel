...
- [ ] Returning `violated` (exit 5) for a capture missing audio / under min duration / blank
      (LUFS ≤ noise floor) / with out-of-bounds trims.
...
- [ ] Each check carries `findings { total, shown, truncated }`; finding lists are capped by
      `DATA_LIMITS`.
- [ ] The contract body performs no IO: all facts (decode, geometry, LUFS, peak, duration) are
      passed in from a thin runner.
...
- `src/reels/contracts/verify.py` → `check(capture: Capture, facts: Facts) -> Report` where
  ...
  { name, ok: bool, value?, findings: {total,shown,truncated} }`.
- `src/reels/media.py` → `probe_facts(path: Path) -> Facts` (runs `ffprobe`; decodes level via
  `ffmpeg volumedetect`/`ebur128` for LUFS/peak; raises `Exit(4)` or returns a
  not-available sentinel for unverifiable).
- `reels verify` sets the process exit per the taxonomy (0/5/7).
